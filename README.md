...
University-TAMER adapts the TAMER fusion checkpoint trained on HME100K to
handwritten university mathematics from MathWriting. The baseline adds:

- strict HME100K-vocabulary normalization with zero-OOV filtering;
- canonical-label-disjoint 10K/1K/1K train/validation/test splits;
- dynamic paper, lighting, perspective and phone-camera augmentation;
- 40% HME100K replay to limit catastrophic forgetting;
- retention-aware checkpoint selection;
- unified ExpRate, <=1, <=2, Token Error Rate and Valid-LaTeX metrics.

The repository intentionally excludes datasets, generated images and model
checkpoints. See [BASELINE_PROJECT.md](BASELINE_PROJECT.md) for dataset
preparation, training, evaluation and ablation commands.

This project extends the original
[TAMER implementation](https://github.com/qingzhenduyu/TAMER) associated with
the AAAI 2025 paper below.
